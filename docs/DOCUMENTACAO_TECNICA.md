...
- **Mobile** (`mobile/`) — Aplicação React Native com Expo Router, espelhando os fluxos públicos do painel web (autenticação, exploração de conteúdos, quiz, fórum, perfil), sempre em tema claro para consistência visual com o web.
...
| **Quiz** | `GET/POST /quizzes`, `GET /quizzes/:id`, `POST /quizzes/:id/attempts`, `GET /quizzes/:id/ranking` | Criação restrita a ADMIN; correcção de tentativas sempre no servidor; ranking sem duplicados, com penalização por repetição |
